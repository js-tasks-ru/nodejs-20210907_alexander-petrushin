  ...
  validate(obj) {
    ...
    for (const field of Object.keys(this.rules)) {
      ...
      if (type !== 'string' && type !== 'number') {
        errors.push({field, error: `expect string or number, got ${type}`});
        return errors;
      }
      ...
      switch (type) {
        ...
        case 'number':
          if (isNaN(value)) {
            errors.push({field, error: `expected number, got NaN`});
          }
          if (value === Math.abs(Infinity)) {
            errors.push({field, error: `expected number, got Infinity`});
          }
          if (value < rules.min) {
